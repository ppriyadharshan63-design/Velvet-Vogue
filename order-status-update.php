...
try {
    ...
    if ($tracking_number && $tracking_number !== ($order['tracking_number'] ?? '')) {
        ...
        $tracking_comment = 'Tracking number updated: ' . $tracking_number;
        if ($comment) {
            $tracking_comment = $comment . ' (Tracking: ' . $tracking_number . ')';
        }
        addOrderStatusHistory($conn, $order_id, $order['status'], $tracking_comment, $admin_id);
        
        $updated = true;
    }
    
    // Add comment only if no other updates
    if (!$updated && $comment) {
